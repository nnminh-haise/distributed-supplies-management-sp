ALTER PROCEDURE [dbo].[SP_REPORT_OF_PERCENTAGE_OF_IMPORT_AND_EXPORT]
    @FROM_DATE DATETIME,
    @TO_DATE DATETIME
AS
BEGIN
    ...
    SELECT 
        ...
        ISNULL(TOTAL_EXPORT_DETAILS.XUAT, 0) AS XUAT,
        ISNULL(TOTAL_EXPORT_DETAILS.TI_LE_XUAT, 0) AS TI_LE_XUAT
    FROM (
        SELECT
            PN.NGAY AS NGAY,
            SUM(PN.SOLUONG * PN.DONGIA) AS NHAP,
            SUM(PN.SOLUONG * PN.DONGIA) * 100.0 / (SELECT SUM(SOLUONG * DONGIA) FROM #TEMP_IMPORT) AS TI_LE_NHAP
        FROM #TEMP_IMPORT AS PN
        GROUP BY PN.NGAY
    ) AS TOTAL_IMPORT_DETAILS
    FULL OUTER JOIN (
        SELECT
            PX.NGAY AS NGAY,
            SUM(PX.SOLUONG * PX.DONGIA) AS XUAT,
            SUM(PX.SOLUONG * PX.DONGIA) * 100.0 / (SELECT SUM(SOLUONG * DONGIA) FROM #TEMP_EXPORT) AS TI_LE_XUAT
        FROM #TEMP_EXPORT AS PX
        GROUP BY PX.NGAY
    ) AS TOTAL_EXPORT_DETAILS
